...
router.get("/:agenda_no", [auth], async (req, res) => {
  const answers = new Answers(req.user.db);
  const data = await answers.getOne(req.params.agenda_no);
  answers.close();

  if (!data || data.length == 0)
    return res
      .status(404)
      .send(`The answer with the no ${req.params.agenda_no} does not exist`);
